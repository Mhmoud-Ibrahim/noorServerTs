...
// });
import { Order } from '../../../database/models/order.model.js';
import { CartModel } from "../../../database/models/cart.model.js"; // تأكد من المسار
import { ProductModel } from "../../../database/models/product.model.js";
import type { NextFunction, Request, Response } from "express";
import { catchError } from "../../middleware/catchError.js";
import { AppError } from "../../utils/appError.js";
...
// 3. تقرير المبيعات اليومي (الأرباح والمبيعات)
export const getDailyReport = catchError(async (req: Request, res: Response, next: NextFunction) => {
    const startOfDay = new Date();
    startOfDay.setHours(0, 0, 0, 0);
    const endOfDay = new Date();
    ...
    const report = await Order.aggregate([
        ...
        { $unwind: "$orderItems" },
        {
            $group: {
                ...
                totalCost: { $sum: { $multiply: ["$orderItems.costPrice", "$orderItems.quantity"] } },
...
// 4. إلغاء الطلب وإرجاع الكميات للمخزن
export const cancelOrder = catchError(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const order = await Order.findById(id);
    if (!order) return next(new AppError("الطلب غير موجود", 404));
...
export const getAllOrders = catchError(async (req: Request, res: Response, next: NextFunction) => {
    const mongooseQuery = Order.find()
        ...
        .populate('orderItems.product', 'title imageCover');
    ...
    res.status(200).json({
        message: "success",
        metadata: { page: Number(req.query.page) || 1, results: orders.length },
        orders
    });
});
