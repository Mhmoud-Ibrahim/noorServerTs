...
//add product
const addCategory = catchError(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (isExist) return next(new AppError("product already exist", 400));
    const category = await CategoryModel.create({ ...req.body });
    res.status(201).json({ message: "success", category });
});
// get all products
const getAllCategories = catchError(async (req: Request, res: Response, next: NextFunction) => {
 ...
 // git one product
 const getOneCategory = catchError(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
 ...
 // delete product
 const deleteCategory = catchError(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
...
// update product
const updateCategory = catchError(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
